...
const AdminWardEditPage = ({ user }) => {
    ...
    const fetchDetail = async function() {
        let params = {
            router,
            path: `/api/v1/admin/zone-wards/${id}`,
            token: user.token,
        };
        let res = await ApiCall(params);
        if(!res.status) return showError(res);
        ...
        setDistrictValue(res.data.zoneDistrict)
    };
    ...
    return (
        <LayoutPage header={header} user={user}>
            <Section borderBottom={true} style={{ padding: "2rem 0" }}>
                ...
                {districtValue && zoneDistrictData && zoneDistrictData.length ?
                <HorizontalList itemSize={ListItemSize.STRETCH}>
                    <ListItem style={{ marginRight: "1rem", width: "50%" }}>
                        ...
                        >
                            {zoneDistrictData.map(district => (
                                <Option key={district.id} value={district.id}>{district.name.vi}</Option>
                            ))}
                        </Select>
                    </ListItem>
                </HorizontalList> : ''}
...
export default AdminWardEditPage;
