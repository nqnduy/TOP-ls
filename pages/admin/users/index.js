...
const AdminUsersPage = ({ user, children }) => {
    ...
    const handleSearch = function(isRest = false, isExport = false) {
        ...
        if(isExport) {
            let exportUrl = `${CONFIG.NEXT_PUBLIC_API_BASE_PATH}/api/v1/admin/users?get=true&export=true&${currentFilter}&token=${user.token}`;
    ...
    const header = (
        <PageHeader pretitle="admin" title={`User (${pagination.total || 0})`} button={createBtn} separator={true}>
            List
        </PageHeader>
    );
