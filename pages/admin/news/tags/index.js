...
const AdminTagsPage = ({ user, children }) => {
	...
	//Permissions
	const canList = checkPermission(user, "tag_list");
	...
	const canDelete = checkPermission(user, "tag_delete");
	//
	...
	const fetchList = async function (pagination = null, filters = null, sorter = null) {
		...
		let params = {
			router,
			path: `/api/v1/admin/tags?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}&${filtera}`,
			token: user.token,
		};
	...
	const handleSearch = function (isRest = false, isExport = false) {
		...
		if (isExport) {
			let exportUrl = `${CONFIG.NEXT_PUBLIC_API_BASE_PATH}/api/v1/admin/tags?get=true&export=true&${currentFilter}&token=${user.token}`;
		...
		},
		{
			...
			render: function renderActions(text, row) {
				return (
					<>
						<AdminButton
							onClick={() => router.push(`/admin/news/tags/edit/${row.id}`)}
	...
	const createBtn = (
		<React.Fragment>
			...
			{canCreate ? <AdminButton href="/admin/news/tags/create">Create</AdminButton> : ""}
		</React.Fragment>
	);

	const header = (
		<PageHeader pretitle="admin" title={`Tag (${pagination.total || 0})`} button={createBtn} separator={true}>
			List
		</PageHeader>
	);
