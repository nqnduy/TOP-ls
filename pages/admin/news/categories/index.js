...
const AdminPostCategoriesPage = ({ user, children }) => {
	...
	const handleSearch = function (isRest = false, isExport = false) {
		if (isRest) {
			...
			formSearchRef.current.title.value, (formSearchRef.current.active.value = "");
			setFormSearchInput({
				...formSearchInput,
				createdFrom: "",
				createdTo: "",
		...
		if (isExport) {
			let exportUrl = `${CONFIG.NEXT_PUBLIC_API_BASE_PATH}/api/v1/admin/post-categories?get=true&export=true&${currentFilter}&token=${user.token}`;
		...
		},
		{
			...
			render: function renderActions(text, row) {
				return (
					<>
						<AdminButton
							onClick={() => router.push(`/admin/news/categories/edit/${row.id}`)}
	...
	const createBtn = (
		<React.Fragment>
			...
			{canCreate ? <AdminButton href="/admin/news/categories/create">Create</AdminButton> : ""}
		</React.Fragment>
	);
