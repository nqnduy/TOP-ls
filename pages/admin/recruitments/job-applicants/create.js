...
import { 
    showMessages, showSuccess, showError,
    checkPermission, postGetForm, preSaveForm
...
const AdminJobApplicantsCreatePage = ({ user }) => {
                    ...
                    ? <HorizontalList itemSize="stretch">
                        <ListItem style={{ marginRight: "1rem" }}>
                            <InputSelect
                                ...
                                label={<label style={{ display: "inline-block" }}>Job <span style={{color: "red"}}>*</span></label>}
                                labelInValue
                            ...
                            >
                                {jobList.map(function(job) {
                                    return(
                                        <Select.Option key={job.id} value={job.id}>{getObjectTrans(job.title)}</Select.Option>
                                    )
                                })}                            
                            </InputSelect>
                        </ListItem>
                    ...
                    ? <HorizontalList itemSize="stretch">
                        <ListItem style={{ marginRight: "1rem" }}>
                            <InputSelect
                                ...
                                label={<label style={{ display: "inline-block" }}>Province <span style={{color: "red"}}>*</span></label>}
                                labelInValue
                            ...
                            >
                                {provinceList.map(function(zoneProvince) {
                                    return(
                                        <Select.Option key={zoneProvince.id} value={zoneProvince.id}>{getObjectTrans(zoneProvince.name)}</Select.Option>
                                    )
                                })}                            
                            </InputSelect>
                        </ListItem>
                    ...
                    <ListItem style={{ marginRight: "1rem" }}>
                        ...
                        >
                            {Object.keys(ApplicantStatusTrans()).map(function(status) {
                                return (
                                    <Select.Option key={status} value={status}>{ApplicantStatusTrans()[status]}</Select.Option>
                                )
                            })}
                        </InputSelect>
                    </ListItem>
...
export default AdminJobApplicantsCreatePage;
