...
const AdminJobContractsPage = ({ user, children }) => {
    ...
    //Permissions
    const canList   = checkPermission(user, 'job_contract_list');
    const canCreate = checkPermission(user, 'job_contract_add');
    const canEdit   = checkPermission(user, 'job_contract_edit');
    const canDetail = checkPermission(user, 'job_contract_detail');
    const canDelete = checkPermission(user, 'job_contract_delete');
    //
    ...
    const handleSearch = function(isRest = false, isExport = false) {
        ...
        if(isExport) {
            let exportUrl = `${CONFIG.NEXT_PUBLIC_API_BASE_PATH}/api/v1/admin/job-contracts?get=true&export=true&${currentFilter}&token=${user.token}`;
        ...
        },
        {
            ...
            render: function renderActions(text, row) {
                return (
                    <>
                        <AdminButton
                            onClick={() => router.push(`/admin/recruitments/job-contracts/edit/${row.id}`)} 
    ...
    const createBtn = (
        <React.Fragment>
            ...
            {canCreate ? <AdminButton href="/admin/recruitments/job-contracts/create">Create</AdminButton> : ''}
        </React.Fragment>
    );

    const header = (
        <PageHeader pretitle="admin" title={`Contract (${pagination.total || 0})`} button={createBtn} separator={true}>
            List
        </PageHeader>
    );
...
export default AdminJobContractsPage;
