        ...
        <>
         ...
         `}</style>
            <div className="tagsButton">
                <Button {...STYLE} active={currentChoosingTagIndex.find((item) => item == -1)} key={-1} onClick={(item2) => {
                    onClickTag(-1);
                }}>All Filter
                </Button>
                {tags.map((tag, index) => {
                    return <Button {...STYLE} active={currentChoosingTagIndex.find((item) => item == index) >= 0} key={index}
                        onClick={(item2) => {
                            onClickTag(index);
                        }}
                    >{tag}
